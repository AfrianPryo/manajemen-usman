    ...
    @if($showCreateModal)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-neutral-900/50 backdrop-blur-sm p-4 overflow-y-auto">
            <div class="bg-white dark:bg-slate-800 w-full max-w-lg rounded-sm border border-neutral-200 dark:border-slate-700 shadow-2xl overflow-hidden my-8 animate-in fade-in zoom-in duration-150">
                ...
                {{-- Modal Form Body --}}
                <form wire:submit="{{ isset($isEditing) && $isEditing ? 'updateUser' : 'save' }}" class="p-6">
                    ...
                    <x-slot:tab2>
                        ...
                        <p class="flex items-start gap-2 text-[11px] leading-relaxed text-neutral-500 dark:text-neutral-400">
                            ...
                            @else
                                <span>Perubahan data tidak mereset password. Gunakan <strong class="font-semibold text-neutral-700 dark:text-neutral-200">Reset PW</strong> jika pengguna lupa password.</span>
                            @endif
                        </p>
                    </x-slot:tab2>
                    <x-slot:submit>
