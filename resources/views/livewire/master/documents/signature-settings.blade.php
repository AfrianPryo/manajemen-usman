<div class="w-full max-w-3xl mx-auto space-y-5 text-neutral-800 dark:text-neutral-100 px-4 py-4 sm:px-6 font-sans">
    ...
    <div class="bg-white dark:bg-slate-800 rounded-sm border border-neutral-100 dark:border-slate-700 shadow-sm shadow-black/[0.02] p-5 space-y-4">
        <h2 class="text-sm font-bold text-neutral-900 dark:text-white tracking-tight pb-3 border-b border-neutral-100 dark:border-slate-700">{{ $editingId ? 'Edit Profil' : 'Tambah Profil Tanda Tangan' }}</h2>
        ...
        <label class="flex items-center gap-2 text-xs font-medium text-neutral-600 dark:text-neutral-300">
            <input type="checkbox" wire:model="is_default" class="rounded border-neutral-300 text-blue-900 focus:ring-blue-400">
            Jadikan tanda tangan default
        </label>

        <div class="flex justify-end gap-2 pt-2 border-t border-neutral-100 dark:border-slate-700">
            @if ($editingId)
                <button wire:click="$set('editingId', null)" class="px-4 py-2 text-xs font-semibold text-neutral-600 dark:text-neutral-300 bg-neutral-100 dark:bg-slate-700 rounded-sm hover:bg-neutral-200 dark:hover:bg-slate-600 transition-all">Batal</button>
            @endif
                    ...
                    @forelse ($signatures as $sig)
                        <tr wire:key="sig-{{ $sig->id }}" class="hover:bg-neutral-50/60 dark:hover:bg-slate-700/30 transition-colors">
                            ...
                            <td class="px-4 py-3 text-center">
                                @if ($sig->is_default)
                                    <span class="px-2.5 py-0.5 text-[10px] font-semibold rounded-sm bg-emerald-50 dark:bg-emerald-950/50 text-emerald-600 dark:text-emerald-400 border border-emerald-200/60 dark:border-emerald-800">Default</span>
                                @else
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center justify-center gap-1">
                                    <button wire:click="edit({{ $sig->id }})" class="p-1.5 text-amber-600 hover:text-amber-800 dark:text-amber-400 hover:bg-amber-50 dark:hover:bg-amber-950/40 rounded-sm transition-all" title="Edit">
                                        <x-heroicon-o-pencil-square class="w-4 h-4" />
                                    </button>
                                    <button type="button" x-on:click.prevent="$store.confirmDialog.open({
                                            message: 'Yakin hapus profil ini?',
                                            confirmText: 'Ya, Hapus',
                                            onConfirm: () => $wire.delete({{ $sig->id }})
